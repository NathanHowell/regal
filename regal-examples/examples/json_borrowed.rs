...
enum Value<'src> {
    ...
    String(&'src str),
    Array(Vec<Value<'src>>),
    Object(HashMap<&'src str, Value<'src>>),
}
...
impl<'src> TokenCursor<'src> {
    fn new(tokens: &'src [SpannedToken<'src>], source_len: usize) -> Self {
        Self {
            tokens,
            index: 0,
            source_len,
    ...
    fn next(&mut self) -> Option<&'src SpannedToken<'src>> {
    ...
    fn peek(&self) -> Option<&'src SpannedToken<'src>> {
...
fn lex<'src>(source: &'src str) -> Result<'src, Vec<SpannedToken<'src>>> {
...
fn parse_value<'src>(cursor: &mut TokenCursor<'src>) -> Result<'src, Value<'src>> {
...
fn parse_array<'src>(cursor: &mut TokenCursor<'src>) -> Result<'src, Value<'src>> {
...
fn parse_object<'src>(cursor: &mut TokenCursor<'src>) -> Result<'src, Value<'src>> {
    ...
    while let Some(token) = cursor.peek() {
        match token.token {
            ...
            Token::String(key) if expect_key => {
                ...
                let value = parse_value(cursor)?;
                map.insert(key.trim_matches('"'), value);
                expect_key = false;
            }
            _ => {
                return Err((
...
fn main() {
    let filename = env::args()
        .nth(1)
        .expect("usage: json-borrowed <path/to/file>");
...
fn report(filename: &str, source: &str, (message, span): Error<'_>) {
